...
# vars() return a dictionary, which means args is a dictionary
...
while True:
    ...
    cv2.imwrite('frames/' + videoName + "-" +
                str(counter).zfill(3) + '.png', frame)
    ...
    if (startSelecting == True):
        ...
        for box in boxes:
            ...
            tracker = TRACKER_TYPE[args["tracker"]]()
    ...
    for box in boxes:
        ...
        txtfile.write('frame:' + str(counter).zfill(3) + ',xmin:' + str(xmin) +
                      ',xmax:' + str(xmax) + ',ymin:' + str(ymin) + ',ymax:' + str(ymax) + '\n')
